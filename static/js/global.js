...
const navDesktopElements = [
	getQuery(".n-foldable1").get(0),
	getQuery(".n-foldable2").get(0),
	getQuery(".n-foldable3").get(0),
	getQuery(".n-foldable4").get(0),
	getQuery(".n-foldable5").get(0)
];
...
getQuery("img").on("dragstart", () => {return false});
getQuery("svg").on("dragstart", () => {return false});
...
navDesktopLinks.forEach((elm, i) => {
	...
	elm.click(e => {
		...
		navDesktopElements[i].hasClass("active") ?
		gsap.to(navDesktopElements[i], navGsap("-50%", "100%")) :
		gsap.to(navDesktopElements[i], navGsap("-50%", 0));

		navDesktopElements[i].hasClass("active") ?
		getQuery("main *").click(closeNavWithMainDesktop) :
		getQuery("main *").off("click", closeNavWithMainDesktop);
...
async function sendNewsletter (data) {
	const result = await post("/post/newsletter/signUp", data)
	.catch(() => {
		const errField = getElm("newsletter-form-error");
...
async function getBlogTitle (number) {
	...
	const elements = [
		getQuery(".n-t li .n-t-details ul").get(2),
		getQuery(".n-m li .n-t-details ul").get(2),
		getQuery(".n-foldable3 ul").get(0)
	];
...
async function getGalleryTitle (number) {
	...
	const elements = [
		getQuery(".n-t li .n-t-details ul").get(3),
		getQuery(".n-m li .n-t-details ul").get(3),
		getQuery(".n-foldable4 ul").get(0)
	];
